...
const Account = () => {
    ...
    const [shownContentName, setShownContentName] = useState<shownContent>(content || shownContent.ACCOUNT_DETAILS);
            ...
            <div className={styles.layout}>
                <div className={styles.menu}>
                    <CustomNavLink 
                        ...
                        leftSection={<i className="pi pi-user" style={{fontSize: '1.5em'}}/>}
                    />
                    <CustomNavLink 
                        href="/profile/qiuz-history" 
                        onClick={() => setShownContentName(shownContent.QUIZZ_HISTORY)}
                        label='Quizz History'
                        leftSection={<i className="pi pi-chart-bar" style={{fontSize: '1.5em'}}/>}
                    />
                    <CustomNavLink
                        href='/profile/create-quizz'
                        onClick={() => setShownContentName(shownContent.CREATE_QUIZZ)}
                        label='Create Quizz'
                        leftSection={<i className="pi pi-plus" style={{fontSize: '1.5em'}}/>}
                    />
                    <CustomNavLink
                        href='/profile/my-quizzes'
                        onClick={() => setShownContentName(shownContent.MY_QUIZZES)}
                        label='My Quizzes'
                        leftSection={<i className="pi pi-list" style={{fontSize: '1.5em'}}/>}
                    />
                    <CustomNavLink
                        href='/profile/statistics'
                        onClick={() => setShownContentName(shownContent.STATISTICS)}
                        label='Statistics'
                        leftSection={<i className="pi pi-chart-line" style={{fontSize: '1.5em'}}/>}
                    />
                </div>
                <div className={styles.content}>
                    {shownContentName === shownContent.ACCOUNT_DETAILS && <AccountDetails />}
                </div>
            </div>
        </div>
    );
